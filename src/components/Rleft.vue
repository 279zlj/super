<template>
  <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12" id="Rleft">
    <div class="one" :class="isse==='概览'?'two':''" @click="changenav('概览')">
      <div>
        <router-link :to="{name:'Resources'}" ><p >{{$t('message.General')}}</p></router-link>
      </div>
    </div>
    <div class="one" :class="isse==='节点'?'two':''" @click="changenav('节点')">
      <div>
        <router-link :to="{name:'Rosd'}" ><p >{{$t('message.io-agent')}}</p></router-link>
      </div>
    </div>
    <div class="one" :class="isse==='存储池'?'two':''" @click="changenav('存储池')">
      <div>
        <router-link :to="{name:'Rpool'}" ><p >{{$t('message.Tank')}}</p></router-link>
      </div>
    </div>
    <div class="one" :class="isse==='块设备'?'two':''" @click="changenav('块设备')">
      <div>
